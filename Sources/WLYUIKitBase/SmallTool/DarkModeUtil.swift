...
public class JKDarkModeUtil {
    ...
    public static var isLight: Bool {
        ...
        if let value = UserDefaults.userDefaultsGetValue(key: JKLightDarkKey) as? Bool {
            return value
        }
        return true
    }
    ...
    public static var isFollowSystem: Bool {
        if #available(iOS 13, *) {
            if let value = UserDefaults.userDefaultsGetValue(key: JKDarkToSystemKey) as? Bool {
                return value
            }
            return true
        }
    ...
    public static var isSmartPeeling: Bool {
        if let value = UserDefaults.userDefaultsGetValue(key: JKSmartPeelingKey) as? Bool {
            return value
        }
        return false
    }
    ...
    public static var SmartPeelingTimeIntervalValue: String {
        get {
            if let value = UserDefaults.userDefaultsGetValue(key: JKSmartPeelingTimeIntervalKey) as? String {
                return value
            }
            return "21:00~8:00"
        }
        set {
            UserDefaults.userDefaultsSetValue(value: newValue, key: JKSmartPeelingTimeIntervalKey)
        }
    }
}
...
public extension JKDarkModeUtil {
    ...
    static func setDarkModeFollowSystem(isFollowSystem: Bool) {
        if #available(iOS 13.0, *) {
            // 1.1、设置是否跟随系统
            UserDefaults.userDefaultsSetValue(value: isFollowSystem, key: JKDarkToSystemKey)
            let result = UITraitCollection.current.userInterfaceStyle == .light ? true : false
            UserDefaults.userDefaultsSetValue(value: result, key: JKLightDarkKey)
            UserDefaults.userDefaultsSetValue(value: false, key: JKSmartPeelingKey)
            // 1.2、设置模式的保存
            if isFollowSystem {
                UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.overrideUserInterfaceStyle = .unspecified
            } else {
    ...
    static func setDarkModeCustom(isLight: Bool) {
        if #available(iOS 13.0, *) {
            ...
            // 1.2、设置跟随系统和智能换肤：否
            UserDefaults.userDefaultsSetValue(value: false, key: JKDarkToSystemKey)
            UserDefaults.userDefaultsSetValue(value: false, key: JKSmartPeelingKey)
            // 1.3、黑白模式的设置
            UserDefaults.userDefaultsSetValue(value: isLight, key: JKLightDarkKey)
        } else {
            UserDefaults.userDefaultsSetValue(value: false, key: JKSmartPeelingKey)
            // 模式存储
            UserDefaults.userDefaultsSetValue(value: isLight, key: JKLightDarkKey)
    ...
    static func setSmartPeelingDarkMode(isSmartPeeling: Bool) {
        if #available(iOS 13.0, *) {
            // 1.1、设置智能换肤
            UserDefaults.userDefaultsSetValue(value: isSmartPeeling, key: JKSmartPeelingKey)
    ...
    static func setSmartPeelingTimeChange(startTime: String, endTime: String) {
        ...
        if #available(iOS 13.0, *) {
            ...
            // 1.2、黑白模式的设置
            UserDefaults.userDefaultsSetValue(value: light, key: JKLightDarkKey)
        } else {
            // 模式存储
            UserDefaults.userDefaultsSetValue(value: light, key: JKLightDarkKey)
            // 通知模式更新
...
public extension JKDarkModeUtil {
    ...
    static func isSmartPeelingTime(startTime: String? = nil, endTime: String? = nil) -> Bool {
        ...
        if startTime != nil && endTime != nil {
            ...
            timeIntervalValue = JKDarkModeUtil.SmartPeelingTimeIntervalValue.separatedByString(with: "~") as! [String]
        }
        // 1、时间区间分隔为：开始时间 和 结束时间
        // 2、当前的时间转时间戳
        let currentDate = Date()
        let currentTimeStamp = Int(currentDate.dateToTimeStamp())!
        let dateString = currentDate.toformatterTimeString(formatter: "yyyy-MM-dd")
        let startTimeStamp = Int(Date.formatterTimeStringToTimestamp(timesString: dateString + " " + timeIntervalValue[0], formatter: "yyyy-MM-dd HH:mm", timestampType: .second))!
        var endTimeStamp = Int(Date.formatterTimeStringToTimestamp(timesString: dateString + " " + timeIntervalValue[1], formatter: "yyyy-MM-dd HH:mm", timestampType: .second))!
